...
class Trainer():
    ...
    def __init__(
        ...
        if data_type=='p_time':
            ...
            all_tfs=pd.read_csv(self.tfs_path)
            ...
            if type(min_cells)!=list or len(min_cells)!=2:
                sys.exit('In 2_time mode, you should provide a list contains two min_cells parameters to filter two adata files.')
    ...
    def getMetaData(self,cell_generate_per_time,cell_select_per_time) -> None:
        ...
        for i in range(cell_generate_per_time):
            cell_indexes=random.sample(range(self.input_data.shape[0]),cell_select_per_time)
    ...
    def plot_scatter(self,type: str = 'raise',num_rows: int=2,num_cols:int=5,fig_width:int=20,fig_height:int=8,)-> None:
        """
    ...
    def generate_data_2_time(self,cell_generate_per_time,cell_select_per_time):
        ...
        # normalize data 
        self.input_data=(self.input_data-self.input_data.mean(axis=0))/(self.input_data.max(axis=0)-self.input_data.min(axis=0))
        self.output_data=(self.output_data-self.output_data.min(axis=0))/(self.output_data.max(axis=0)-self.output_data.min(axis=0))
    ...
    def generate_data_p_time(self,cell_divide_per_time, cell_select_per_time, cell_generate_per_time) -> None:
        """
        Generate data in the 2_time mode.
        ...
        for i in range(1,len(self.origin_data)):
            for j in range(cell_generate_per_time):
                random_idxs=random.sample(range(len(self.origin_data[i])),cell_select_per_time)
        ...
        print(f"{(len(self.origin_data)-1)} groups of new data were generated, each with {cell_generate_per_time} meta cells.")
    ...
    def get_one_hot(self,) -> None:
        ...
        """
        vectorizer=CountVectorizer(vocabulary=self.genes.tolist(),lowercase=False)   # lowercase gene names
    ...
    def sort_idx(self, cell_divide_per_time) -> np.array:
        ...
        middle_idx=list(self.adata.obs.index[len(ptime_0_idx):len(self.adata)-len(ptime_1_idx)])
        sub_length=cell_divide_per_time
        sub_index=[middle_idx[i:i+sub_length] for i in range(0, len(middle_idx), sub_length)][:-1]

        print(f'{len(self.adata)} cells were divided into {len(sub_index)} groups according to the pseudo-time, with {cell_divide_per_time} cells in each group. The first and last cells are discarded.')
